...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const rows = await serviceSelect(
      'messages',
      `select=id,customer_id,message_type,body,status,ai_status,send_status,error_message,created_at,customers(name,last_visit_at,visit_count)&store_id=eq.${storeFilter}&message_type=eq.return_visit&order=created_at.desc&limit=50`
    );
